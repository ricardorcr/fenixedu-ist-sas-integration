...
<div class="col-sm-4">
	...
	<select id="ingressionRegimeMapping_ingression" class="js-example-basic-single" name="ingression" multiple="multiple">
	</select>

	<script>
		<%--$("#ingressionRegimeMapping_ingression").val('<c:out value='${not empty param.ingression ? param.ingression : ingressionRegimeMapping.ingressionType.iterator().next().externalId }'/>');--%>
		$(document).ready(function() {
            ingressionValues = [
			<c:forEach items="${ingressionValues}" var="ingressionType">
            {
                	text : "<c:out value='${ingressionType.localizedName}'/>",
                    id : "<c:out value='${ingressionType.externalId}'/>",
                    selected : ${ingressionRegimeMapping.getIngressionTypeSet().contains(ingressionType)}
                    // selected: ingressionRegimeMapping.ingressionTypeSet.contains(ingressionType)
            },
            </c:forEach>
			];
            
            $("#ingressionRegimeMapping_ingression").select2(
                {
                    data : ingressionValues,
                }
            );
		});
	</script>
