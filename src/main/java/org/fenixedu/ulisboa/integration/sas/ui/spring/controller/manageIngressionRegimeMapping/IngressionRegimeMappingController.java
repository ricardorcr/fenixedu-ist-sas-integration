...
public class IngressionRegimeMappingController extends SasBaseController {
    ...
    public String create(Model model) {
        model.addAttribute("ingressionValues", Bennu.getInstance().getIngressionTypesSet().stream().sorted(
                Comparator.comparing(IngressionType::getDescription)).collect(Collectors.toList()));
    ...
    public String create(@RequestParam(value = "ingression", required = false) IngressionType[] ingressionType,
            @RequestParam(value = "regimeCode", required = false, defaultValue = "#{new IngressionType[0]}") String regimeCode,
            @RequestParam(value = "regimeCodeWithDescription", required = false) String regimeCodeWithDescription,
            Model model, RedirectAttributes redirectAttributes) {

        try {
            createIngressionRegimeMapping(new HashSet<>(Arrays.asList(ingressionType)), regimeCode, regimeCodeWithDescription);
    ...
    @Atomic
    public SasIngressionRegimeMapping createIngressionRegimeMapping(Set<IngressionType> ingressionTypeSet, String regimeCode,
            String regimeCodeWithDescription) {
        return SasIngressionRegimeMapping.create(ingressionTypeSet, regimeCode, regimeCodeWithDescription);
    }
    ...
    public String update(@PathVariable("oid") SasIngressionRegimeMapping ingressionRegimeMapping, @RequestParam(value = "ingression",
            required = false, defaultValue = "#{new IngressionType[0]}") IngressionType[] ingressionTypes,
        ...
        try {
            ...
            updateIngressionRegimeMapping(new HashSet<>(Arrays.asList(ingressionTypes)), regimeCode, regimeCodeWithDescription,
                    model);
    ...
    @Atomic
    public void updateIngressionRegimeMapping(Set<IngressionType> ingressionTypes, String regimeCode, String
            regimeCodeWithDescription, Model model) {
        getIngressionRegimeMapping(model).edit(ingressionTypes, regimeCode, regimeCodeWithDescription);
    }
